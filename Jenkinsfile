pipeline {
  ...
  stages {
    ...
    stage('Deploy to Dev') {
      ...
      steps {
        container('docker-tools') {
          sh 'docker run -t docker.io/schoolofdevops/argocd-cli:latest argocd app sync dso-demo --insecure --server $ARGO_SERVER --auth-token $AUTH_TOKEN'
          sh 'docker run -t docker.io/schoolofdevops/argocd-cli:latest argocd app wait dso-demo --health --timeout 300 --insecure --server $ARGO_SERVER --auth-token $AUTH_TOKEN'
        } 
      }   
    }
  }
